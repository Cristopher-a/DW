...
def  recov(page: ft.Page):
    ...
    def co(e):
    # Revisar si todos los campos tienen valores
       if txt1.value and txt2.value and txt3.value and txt4.value:
           btn.disabled = False  # Habilitar el botón si todos tienen un valor
       else:
           btn.disabled = True   # Deshabilitar el botón si falta algún valor

       page.update()  # Actualizar la página para reflejar los cambios
 
# Definir los TextFields
    ...
    btn= ft.ElevatedButton("Submit",on_click=r,disabled=True)


    nd=ft.Container(
        ft.Column([
            ft.Row([
                ft.Text("Recovery Page"),
            ]),
            ft.Row([
                txt1,txt2,txt3,txt4
            ]),
    ...
    error = ft.Banner(
        ...
        actions=[ft.TextButton(text="Crear cuenta", style=ft.ButtonStyle(color=ft.colors.WHITE), on_click=lambda _: page.close(error)),
    ...
    num=ft.Container(
        ft.Column([
            ...
            ft.Row([correo_field]),
            ft.Row([
                ft.ElevatedButton("Recuperar", on_click=recover)
            ])
